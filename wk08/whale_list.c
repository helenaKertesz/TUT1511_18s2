...
int main(int argc, char *argv[]) {
    ...
    write_sighting( stdout, first_pod);
    //write_sightings_file("test.txt", first_pod);
...
void write_sighting(FILE *f, struct pod *p){
    // like "<year>/<month>/<day> <num_seem> <species>"
...
struct pod *read_sightings_file(char filename[]) {
    ...
    struct pod * new_pod = read_sighting(f);
    
    while( new_pod != NULL ){
        
        if( first_sighting == NULL ){
            first_sighting = new_pod;
        } else {
            last_sighting->next = new_pod;
            new_pod->next = NULL;
        }
        last_sighting = new_pod;

        new_pod = read_sighting(f);
    }
